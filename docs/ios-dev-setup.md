...
Expo покаже **інтерактивний список** — симулятор або підключений iPhone. Без `--device` Expo бере останній/увімкнений симулятор (наприклад iPhone 17 Pro); `npm run ios` завжди передає `--device`, щоб список з’являвся щоразу.
