...
- **Decision:** Client action logs go through `lib/debug/dev-log.ts`, gated by `__DEV__` (always silent in production) and `EXPO_PUBLIC_LOG_LEVEL` (`none` \| `error` \| `event` \| `detail` \| `all`). Default in dev when unset: `event` (local key actions only). Observed remote peer events require `detail`+. Timings (lexicon, submitWord) require `all`. Format: local wall-clock + player name + action. **Exception: see ADR-025 — when diagnostics are collecting, all `devLogAction` calls (including observed events) are mirrored to the local ring probe even in production (console remains silent in production builds).**
...
- **Decision:** Vote wall-clock expiry and round timer finish stay client-committed (RTDB has no server cron for these). Clients schedule a local wake at `expiresAt` (votes) / `timerEndsAt + FINISH_WORD_SUBMIT_GRACE_MS` (RTDB finish — local time-up UI still wakes at `timerEndsAt`) instead of polling every second. Primary resolver is the lexicographically smallest online live-round uid; other online candidates failover after ~1.5s if the vote/status is still open. Cast-vote transactions and `reconcileOpenSessionVotes` on presence remain event-driven. Multiple screens on one room share one `subscribeGameSession` RTDB `onValue` (ref-counted).
...
## ADR-023: Nearby peer archive sync (no RTDB history)

- **Decision:** Finished-round room history stays **device-local** (AsyncStorage). When `baseWordRound > 0`, the client may **silently** pull stripped archives from nearby peers over **LAN UDP+TCP first**, then **BLE GATT** (`munim-bluetooth`) if gaps remain (`isAvailable` = LAN ∨ BLE). Host advertises on **both** when hosting **and BLE capability is allowed**. **Permission gates are split:** LAN and BLE are requested/evaluated independently — BT deny must not set global nearby `denied` or block LAN; hybrid skips BLE when capability is `denied` (probe cannot reopen); `unknown` may probe on lobby/join. **iOS:** join/browse may set `os-pending` with BLE left `unknown` until a real BLE host/fetch confirms capability — play must not hydrate into BLE (no mid-round OS BT prompt). Sync uses explicit `allowBleProbe` (not `allowOsPermissionPrompt`); join→play coalesce ANDs probe and a live `bleProbeLiveAllowed` / `isBlePhaseStillAllowed` re-check before the hybrid BLE phase so mid-play cannot open BLE even if join/browse sync is already in-flight; lobby may probe. Client **Wants full prior rounds `0..min(N, MAX_ROUNDS_PER_ROOM)-1`** (constant `12`) and contacts discovered hosts so TCP/BLE HaveAck ∩ **actually served** can stop lobby advertise. **Residual:** a partial-archive host may keep advertising if peers complete via another host (∩ served never reaches `isComplete` on that host). UDP discovery: announce destination === listen port (`reusePort`). TCP/BLE: one archive per frame + `archivesEnd`; BLE TX is serialized with single-flight Want serve (shared GATT notify). UDP HaveAck without Want remain untrusted; TCP/BLE after Want are trusted. Play QR forces advertise while open; closing QR stops host — **v1: no play advertise without QR**. Play sync does not show a new OS permission prompt (join/browse may). Lobby: sync-then-host. **Store:** Local Network + Bluetooth (iOS `NSBluetooth*`; Android BT scan/advertise/connect + `NEARBY_WIFI_DEVICES`; location for API<31 BLE). iOS `os-pending` until successful import/completion handshake. Imports match `gameId` + gaps. **Profile stats are not backfilled from peer archives in v1**. No payload encryption. **Threat model (v1):** family/trusted nearby; hostile roster-uid spoof residual. **Deferred:** lobby browse without code; crypto peer auth; peer stats attest; play advertise without QR; multi-central BLE session ids. The room round cap product UX (rematch stop at `MAX_ROUNDS_PER_ROOM`) shipped separately — see **ADR-024**.
- **Alternatives considered:** Want-only-gaps; BLE advertise-only warm-up (no GATT bytes); ble-plx without peripheral; all-or-nothing BT blocking LAN; multi-archive single TCP line; client `bind(0)` for UDP scan; RTDB history; peer stats finalize.
- **Why rejected:** Partial-Want never reached `isComplete`; advertise-only BLE delivered no bytes; BT deny blocking LAN; `bind(0)` missed discovery port; single TCP line broke multi-round; free-tier/purge; fabricable peer stats.
- **Date:** 2026-08 — `lib/online/nearby/**`, `hooks/useNearbyArchiveSync.ts`
...
## ADR-025: RTDB traffic diagnostics (developer mode)

- **Decision:** Hidden 7-tap gesture on `AppVersionLabel` toggles `developerModeEnabled`. When also `rtdbDiagnosticsEnabled` (Settings), the client instruments all RTDB calls (get, update, remove, onValue) via `rtdb-instrumentation.ts` to estimate JSON payload bytes (down/up). A root layout banner shows a dual sparkline (↓received / ↑sent) and recent actions. Actions from `devLogAction` are mirrored into a local ring probe even in production (prod log level becomes `event` while collecting; all actions, including observed/detail, are mirrored to the probe during collection even in production builds). Timeline actions include round lifecycle (start/finish), votes, and — while collecting — **submitted words** (local via `submitOnlineWord`; observed peer claims from post-seed word-maps child deltas only, skipping `localUid` to avoid double-count) plus **cleared/restored session word maps**. Large JSON ↓ (≥2 KB) may also appear as `rtdb ↓ <path>` timeline rows when callers pass a path into `instrumentedSnapshotVal` (finish/results dig). `clearLocalDataStorage` resets all flags and history. Active room is tracked via `setActiveRoomId` in `subscribeGameSession` and `subscribeSessionWordMaps` (**sticky**): last-listener teardown does **not** clear the room, so play→results remount stays one history group. Explicit leave calls `setActiveRoomId(null)` from `exitOnlineSession` and `navigate-home` (flush + clear banner). Switching to a different room or disabling collecting also flushes (`flushActiveRoom` / room switch). History commits when JSON **or** wire totals are non-zero. Wire bytes come from local Expo module `modules/native-traffic-stats` (autolinked). `RtdbDiagnosticsStore.isHydrated` is set to `true` only after `hydrate()` completes; `RtdbTrafficBanner` gates rendering on this flag to prevent a null → visible flicker on app start when diagnostics were previously persisted as enabled. `pollWireTraffic` self-stops via an `isCollecting()` check at the top of the interval callback, so disabling diagnostics in Settings immediately halts native traffic polling without waiting for the next `record()` call. **Trade-offs:** (1) Cold-start traffic recorded in the first few seconds (before hydration completes) might be missed if diagnostics were previously enabled; (2) Upstream measurement of read-modify-write transactions may double-count bytes if the transaction root matches data already captured in a download measurement (e.g. `session-votes-service`); (3) iOS wire counters are interface-level (OS sandbox), not per-app UID like Android; (4) Word-maps listen-first (ADR-022): child `onChild*` payloads before authoritative seed are **not** counted — only the seed `get` plus post-seed deltas — to avoid ~×2 ↓ on the dominant `wordPlayers` tree; (5) Shared `game_sessions` `onValue` counts **deep JSON deltas** after the first snapshot (finish leaf / presence flip must not re-bill unchanged `players` siblings). Online Home paths use `exitOnlineToHome` / `navigateHomeClearingStack` so sticky room flushes on leave.

- **Alternatives considered:** Firebase Performance monitoring; third-party network interceptors (Flipper/Charles); always-on diagnostics for all users.
- **Why rejected:** Firebase Console Downloads/Storage metrics are not real-time or per-action for clients. Flipper requires debug builds/cables. Diagnostics are for QA/dev profiling on production devices without side-loading. Prod logging is local-only to the probe ring.
- **Date:** 2026-08 — `lib/debug/rtdb-traffic-probe.ts`, `lib/firebase/rtdb-instrumentation.ts`, `modules/native-traffic-stats`, `components/debug/RtdbTrafficBanner.tsx`
