...
| **«Грати ще» (results)**      | Після дії — **свіжий** read RTDB (`optIntoLiveRound`): відсутній root або `finished` → rematch/bootstrap з архіву; маршрут за `resolvePostJoinRoute`. Для вже відкритого rematch `waiting` presence (`rejoin` / online) іде **у фоні** — не блокує вихід з results. Для live `playing` presence **чекаємо** (потрібен `liveRoundPlayerUids`). Лобі не кидає opted-in назад на results при короткому `online:false`. |
...
| Правило                                | Деталі                                                                                                                                                                                                                                                                                                                                                                                            |
| -------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **AppState `background` / `inactive`** | **Play / live:** `inactive` + `background` → `markPlayerOffline` (iOS lock часто лишається на `inactive`). **Waiting lobby / rematch:** лише `background` → offline (`background-only`) — фокус іншого симулятора / Control Center не ставить хибний 📵 у лобі. Тренувальний auto-pause лишається лише на `background`. `markPlayerOffline` пише `online: false` **перед** `onDisconnect.cancel`. |
