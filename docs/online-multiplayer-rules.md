...
| **Eligible**                        | `online === true`, `hasLeft !== true`. У rematch waiting також: `resultsExitedBy` latch / **`baseWordPickerUid`** / `baseWordChosenBy`+слово — **лише** коли `hasLeft !== true` (переживають короткий `online: false`). Свідомий вихід (`hasLeft`) **завжди** виключає з ротації; `leaveGameSession` → `syncLobbyPickerState` передає seat наступному онлайн у черзі. Presence `online: true` **не** викликає `reconcileLobbyPickerState` (очищення слова лише з lobby sync). |
