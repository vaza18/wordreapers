...
| **Вимкнення presence**                 | Unmount без `handoffPlayerPresence` → `voluntaryLeaveWaitingLobbyIfMember`: initial waiting (round 0) non-org → `leaveGameSession`; **rematch waiting** (`baseWordRound > 0`) або latch → лише `markPlayerOffline` (play вимикає presence на round-end, поки peer уже rematch). Handoff між екранами однієї кімнати **не** ставить offline. Explicit offline — `exitOnlineToHome` / `leaveGameSession` / `markPlayerOffline`.                                                                                                                              |
