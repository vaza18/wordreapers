...
| `baseWordRound`                                                                  | Round index (0 = first round); increments on rematch; allowed values `0 .. MAX_ROUNDS_PER_ROOM - 1` (ADR-024); rematch denied when finished on the final index |
...
- `session_word_maps/{gameId}` — **sole client source** of submitted words during play
  - Writes are **per-word**: first finder uses parent `set` on empty `wordPlayers/{normalized}` (`!data.exists()`); second+ use leaf `set(true)` on `/{uid}` (parent rewrite denied so peers cannot be wiped). Client uses **`set`, not `runTransaction`** (TX + default `applyLocally` caused multi-second retries and late `permission_denied` after finish). Leaf/parent creates are **append-only** while `playing`, plus a short **post-finish append grace** (`now < finishedAt + 15000` in `database.rules.json`) so in-flight late submits can land. No mid-round delete. Orphan restore from local cache writes **only the actor’s** leaves (`wordPlayersForUidOnly`); peers restore their own shards. No bulk `session_word_maps` root JSON from clients.
...
- **Online submit write path** (`submitOnlineWord` in `lib/firebase/submit-online-word.ts`): parent/leaf `wordPlayers` shard **`set`** only (no live score TX). Expire finish (`finishGameSessionIfExpired`) waits **`FINISH_WORD_SUBMIT_GRACE_MS`** after `timerEndsAt` so status stays `playing` while in-flight sets complete; play UI still ends locally at `timerEndsAt`. Navigate-to-results drains `syncInFlight` (same grace cap) before archive.
